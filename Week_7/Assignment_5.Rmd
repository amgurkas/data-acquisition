...
```{r loading-raw-txt, class.source='klippy'}
txt <- read_csv(
...
```{r cleaning-variation-col, class.source='klippy'}
df <- txt |> 
...
```{r json-formatting, class.source='klippy'}
# convert df to json
...
website.
```{r html-formatting, class.source='klippy'}
# create html data
html_data <- xtable(df)
# create html file
print.xtable(html_data, type="html", file="CUNYMart.html")
```

### XML
XML is used for data exchange in enterprise applications, web services, and 
configurations. 
```{r xml-formatting, class.source='klippy'}
